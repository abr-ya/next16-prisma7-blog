...
| In Progress | feature-081-outdoor-trip-photo-details-map-focus | outdoor/maps-photos | Make the photo-detail `Show on map` action scroll to and focus the trip map, then visibly fly/zoom to the photo's accepted coordinate while preserving map-layer visibility. Remove the action rather than leave it misleading if a reliable accessible focus flow cannot be delivered. |
