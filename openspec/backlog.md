...
| feature-015 | In Progress | feature-015-file-sharing-structure | files/admin | Add the minimal backend-first file foundation with `FileAsset`, a dedicated UploadThing file route, a basic admin upload form, and the current user's file count. |
| feature-016 | In Progress | feature-016-admin-auth-roles-structure | admin/auth | Document the current registration/authentication setup and plan the role, registration, provider, and admin access model before role-gated features. |
| feature-017 | Planned | feature-017-reserved-next-slice | planning/reserve | Reserved for the next near-term candidate; replace this row before creating an OpenSpec change. |
...
| feature-022 | Cancelled | feature-022-admin-roles-structure | admin/auth | Renumbered and expanded to `feature-016-admin-auth-roles-structure`; do not reuse this number. |
