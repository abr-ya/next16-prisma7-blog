...
| In Progress | feature-079-outdoor-trip-list-viewer-status | outdoor/trips-users | Show each signed-in viewer whether a published trip is their own, one where they are an accepted participant, or another public trip directly on its `/trips` card. |
...
| Ready | outdoor-trip-photo-details-map-focus-action | outdoor/maps-photos | Fix the photo-detail `Show on map` action, which currently only closes the lightbox: it SHALL scroll the trip map into view, focus the photo's accepted coordinate, and visibly zoom/fly toward it while preserving map-layer visibility. If a reliable accessible focus flow cannot be delivered in this slice, remove the action rather than leaving a misleading control. |
