...
| 9 | In Progress | feature-057-outdoor-hike-photo-gallery-viewer | outdoor/hikes-photos | Add true guest-safe thumbnails for hike-linked photos and a signed-in large-window photo viewing experience with near-viewport image sizing and next/previous navigation through the hike photo set. |
| 10 | Candidate | outdoor-photo-reaction-model-exploration | outdoor/photos-social | Explore whether hike-linked photos should support simple likes or a richer rating model, including signed-in/user identity rules, duplicate prevention, public counts, and privacy boundaries. |
| 11 | Candidate | outdoor-hike-to-trip-domain-rename | outdoor/domain | Evaluate and, if accepted, rename the hike-centered domain from `Hike`/`/hikes` toward `OutdoorTrip` or `Trip`, including model names, routes, copy, redirects, specs, and data-preserving migration strategy. Keep `Photo` as an independent reusable entity rather than narrowing it to `TripPhoto`; rename hike-specific associations/actions such as hike-photo ordering to trip terminology; preserve existing data through table/type/column renames rather than drop/create migrations. |
...
| 17 | Candidate | outdoor-track-recording-summary | outdoor/tracks | Show GPX recording start/end date-time and parse/store the GPX `creator` value so public/admin track summaries can display when the route was recorded and which app recorded it when available. |
| 18 | Candidate | outdoor-track-device-metadata-extraction | outdoor/tracks | Investigate and extract recording device details from real GPX creator metadata and vendor-specific extensions after collecting examples from Garmin, Strava, OsmAnd, Komoot, and similar sources. |
| 19 | Candidate | outdoor-hike-combined-track-map | outdoor/maps | Add one map on hike detail pages that renders all linked visible tracks together and frames the combined bounds. |
| 20 | Candidate | outdoor-photo-gps-map-markers | outdoor/maps-photos | Use direct GPS EXIF coordinates to place linked hike photos on the hike map when coordinates exist. |
| 21 | Candidate | outdoor-photo-track-time-matching-spike | outdoor/maps-photos | Investigate and plan timestamp-based photo-to-track matching before implementing interpolation, timezone, and confidence rules. |
| 22 | Candidate | outdoor-photo-track-time-inferred-coordinates | outdoor/maps-photos | Infer candidate photo coordinates from capture timestamps and linked GPX track timelines, with provenance, confidence, and admin approval before public map display. |
