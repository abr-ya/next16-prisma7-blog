...
| feature-070 | feature-070-outdoor-trip-participants | outdoor/trips-users | Add invitation-backed trip participants: trip owners invite existing site users by email, recipients accept or decline from a private inbox, owners/admins cancel or remove, and only accepted membership becomes the base for later public contributions. |
...
| feature-073 | feature-073-workspace-access-policy | auth/workspace-access | Define and enforce the active `user`/`admin` access matrix across the personal `/admin` workspace and administrator controls, including owner and accepted-participant boundaries; subsumes `admin-sidebar-role-sections`. |
...
| feature-088 | feature-088-comments-helper-refactor | comments/public | Pure refactor: migrate the per-target callers (video detail page, trip photo viewer) to the unified `getCommentListItems` helper from feature-087; remove `getPublicVideoCommentListItems` and `getPhotoCommentListItems` plus their orphan row helpers / types; drop the per-domain current-visibility filter from the read path (per-target pages already gate by current visibility, so the filter is redundant). Keep `videoCommentSelect` and `photoCommentSelect` because the mutation helpers still reference them. No observable behavior change for end users. |
