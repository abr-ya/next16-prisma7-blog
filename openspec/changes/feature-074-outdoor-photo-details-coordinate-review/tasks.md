...
- [ ] 1.2 Restrict server-authoritative coordinate-review actions to the photo owner or administrator; revalidate session, published trip/photo linkage, candidate data, and manual-coordinate input before reusing the existing approval/rejection persistence rules.
...
- [ ] 1.4 Add a server-authoritative EXIF extraction/refresh action for a linked published photo that permits only the photo owner or administrator and reuses the existing extraction persistence workflow.
...
- [ ] 2.3 Replace the viewer mutation surface with a photo-owner-or-admin `GPX coordinates` modal that has candidate explanations, approval, rejection, manual override validation, pending states, and clear unavailable-state feedback.
...
- [ ] 2.5 Add compact owner-or-admin `EXIF` and `GPX coordinates` controls to the trip photo card, with focused modals and no controls for read-only viewers.
...
- [ ] 3.2 Extend focused deterministic coverage or checks for owner/admin EXIF refresh, owner/admin coordinate review, denial for trip creator/participant/unrelated actors, and direct-EXIF versus approved-inferred provenance.
