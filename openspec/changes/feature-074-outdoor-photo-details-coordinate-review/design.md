...
- Reuse existing metadata formatting and coordinate-matching/review primitives instead of reimplementing algorithms or duplicating persistence.
- Make review updates immediately consistent across the gallery details and existing map marker layer.

**Non-Goals:**

- Schema changes, public exposure of exact coordinates, EXIF re-extraction controls, or a replacement of the existing administrator trip-management surface.
...
### Extend the gallery viewer with a photo-detail mode and map focus callback

The photo card remains the entry point. Its existing large-photo dialog gains an authorized details affordance, either as an in-dialog panel or a nested detail state, avoiding a separate competing modal. The trip page owns map focus state and passes it to the map and viewer so selecting a valid coordinate pans/zooms the existing map rather than creating a second map implementation.
...
### Reuse existing coordinate review rules through narrow server actions

The detail view will call server actions that re-load session and trip/photo linkage and then apply the existing candidate computation, approval, rejection, and manual override validation. Existing administrator actions may be factored behind authorization-neutral internal helpers, but action entry points shall enforce their distinct allowed roles. No client-supplied coordinate candidate, track data, or authorization result is trusted.
...
- Reusing candidate logic across surfaces risks authorization drift → centralize role checks in server-only helpers and validate direct action calls.

## Migration Plan

1. No database migration is required; consume existing metadata and coordinate fields.
2. Add read/review capability helpers and server-authoritative review action boundaries.
3. Add the gallery details/review UI and connect valid coordinates to existing map focus behavior.
