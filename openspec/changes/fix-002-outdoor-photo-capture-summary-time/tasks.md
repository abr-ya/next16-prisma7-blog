...
- [ ] 1.1 Update photo EXIF summary aggregation to select the first image with an absolute capture instant or valid capture-time provenance; verify a timezone-less `CreateDate` produces summary `localWallTime` and `MISSING` timezone evidence while keeping summary `capturedAt` null.
- [ ] 1.2 Add focused fixture or unit coverage for `CreateDate: 2020:11:07 13:07:17` without an offset; verify it is shown as unconfirmed camera-local time and `ModifyDate` alone remains ineligible.
...
- [ ] 3.1 Run `npm run tsc` and targeted ESLint for changed TypeScript files; verify both pass.
