...
- [ ] 1.1 Add `app/_data/public-comments-feed.ts` exporting `FEED_PAGE_SIZE = 20`, `FeedView = "all" | "mine"`, and `getPublicCommentsFeed({ viewerId, page, view })` returning `{ items: CommentListItem[]; total: number; page: number; pageSize: number; totalPages: number }`; implementation: (a) issue a single `prisma.comment.findMany` with `where: { OR: [{ videoId: { not: null } }, { photoId: { not: null } }] }` and a select that joins `user` (id/name/image), `video` (id/title/thumbnailUrl), and `photo` (id/title + first image fileAsset.url + most recent `hikesToPhotos.hike.slug`); (b) normalize each row to `CommentListItem` via a local adapter that mirrors the shape used by `getPublicVideoCommentListItems` / `getPhotoCommentListItems` — `target.href` is `/videos/{videoId}` for video rows and `/trips/{slug}` for photo rows (skip the row if the photo has no trip link); (c) sort the merged list by `createdAt` descending; (d) when `view === "mine"` and `viewerId` is set, filter to `item.author.id === viewerId`; (e) clamp `page` to `>= 1`, slice to `pageSize`, compute `totalPages`; return `{ items: [], total: 0, page, pageSize, totalPages: 0 }` when the merged list is empty. No current-visibility joins (no `video.visibility = PUBLIC`, no `hike.status = PUBLISHED`) — the feed trusts the creation-time gate.
- [ ] 1.2 Verify the new helper type-checks under `npm run tsc` (no new errors) and that an empty result, a single-page result, and an out-of-range `page` return the documented shapes.
