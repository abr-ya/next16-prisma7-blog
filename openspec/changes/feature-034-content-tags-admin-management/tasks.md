...
- [ ] 1.1 Add a management-oriented content tag query that returns all shared tags with status, slug, total usage count, and grouped post usage.
- [ ] 1.2 Add a rename server action that normalizes name/slug, preserves assignments, rejects slug collisions, and revalidates affected admin/public paths.
- [ ] 1.3 Add an unused-tag delete server action that requires admin access and rejects deletion while supported assignments still exist.
- [ ] 1.4 Review existing status, merge, replace, and assignment-removal actions for management-surface reuse and adjust messages/revalidation where needed.
...
- [ ] 3.3 Ensure all new admin mutations call `requireAdmin()` server-side.
