...
Define admin Tags-page legacy post-tag migration: inventory of remaining legacy-only `Post.tags`, optional drop/rename policy, dry-run, apply, idempotency, dual-write, audit summary, and boundaries for later shared-tag management.
...
- **WHEN** an admin opens `/admin/tags` after this feature
- **THEN** the page SHALL show a placeholder for future shared-tag management when full management tools are not yet shipped
- **AND** that placeholder SHALL state that list, rename, merge, detach, and usage tools are not available yet (or shall be replaced when feature-031 lands)
